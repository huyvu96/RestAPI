...
var Movies={
    ...
    searchAllMovies:async function(query, page , size){
        ...
        var byGenres = "select movies.id from ((movies_genres inner join movies on movies.id = movies_genres.id_movie) inner join genres on genres.id = movies_genres.id_genre) where genres.name_genre like N'%"+query+"%'";
    ...
    getRelatedMovies:async function(idGenre1, idGenre2, idPeople1, idPeople2, page , size){
        var soitem = size;
        var sophantu =(page-1) * soitem;
        ...
        var sql = "select * from movies where (id in ("+ byPeoples +") or id in ("+ byGenres +")) limit "+sophantu+","+ soitem;
        return new Promise(function (resolve, reject)
        {
            db.query(sql.split("undefined").join("null"),[idGenre1, idGenre2, idPeople1, idPeople2], function (err, result)
            {
                if (err)
                {
                    return reject(err)
